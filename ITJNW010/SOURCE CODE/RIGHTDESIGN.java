...
public class RIGHTDESIGN extends JFrame
{
	...
	private void jButton1_actionPerformed(ActionEvent e)
	{
		...
		// TODO: Add any handling code here
		System.out.println("This is for Right to Root Communication");	
			try{
			
			db();
			//System.out.println("After rs1");
			String root="root";
			ResultSet rs = st.executeQuery("Select * from iptabler where locationr='"+root+"'");
		...
		//	int i = 0;
			...
			{
				//i = 1;
				roothost= rs.getString(2);
				System.out.println("From iptabler  " + roothost);
				//break;
			} con.close();
		//	rs.close();			
			toroot = new Socket(roothost,1039);     // from Right to Root Connection        1
			dis2 = new DataInputStream(toroot.getInputStream());
		 	dop2 = new DataOutputStream(toroot.getOutputStream());
			// add your code here
			if(flag==1)
			{
			 dop2.writeUTF("leftrequest");		  // 1st	 left to root
			 dop2.writeUTF(locaton_from_left);    // 2nd
			 dop2.writeUTF(hostname_from_left);   //3rd
			 System.out.println("Left Data Sent to the Right");
			}
			
			
			String s1=InetAddress.getLocalHost().toString();
			StringTokenizer st1=new StringTokenizer(s1,"/");
    		while (st1.hasMoreTokens()) {    
     		 hostname1 = st1.nextToken();
      		System.out.println("Address of local host is : " + hostname1);
			break;
    		}
			
			dop2.writeUTF("rightrequest");   //  1 st  right to root
			dop2.writeUTF("right");			 //2
			dop2.writeUTF(hostname1);		 //3
			System.out.println("Right sent the data to Root");
			//String from_root=dis2.readUTF();
			//System.out.println("From Root: " + from_root);
			System.out.println("Waiting for Acknoledgement...... ");	
			String data_from_root = dis2.readUTF();
			dop1.writeUTF(data_from_root);
			System.out.println("From Root " + data_from_root);
			display=data_from_root;
			
			JOptionPane.showMessageDialog(null,"File or Message Received From Root","Messge Notifier",JOptionPane.OK_OPTION);		
	...
	{
		System.out.println("\njMFT_actionPerformed(ActionEvent e) called.");
